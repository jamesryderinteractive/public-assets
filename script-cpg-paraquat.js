...
function submitForm(event) {
  ...
  var data = {
    ...
    lp_key: input_key,
    lp_action: '',
    lp_subid1: arrAnswers[0],
    lp_subid2: arrAnswers[1],
    lp_subid3: arrAnswers[2],
    phone: phone,
    email: email,
    postal_code: '',
  ...
  fetch(apiUrl, {
    ...
    .then((response) => {
      if (response.ok) {
        if (refParam) {
          window.location.href = '/thank-you-paraquat/?ref=' + refParam;
        } else {
          window.location.href = '/thank-you-paraquat/';
        }
      } else {
